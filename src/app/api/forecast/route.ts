...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      analysisMode = 'fast', // Mode selection: 'fast' (2-3 min) or 'comprehensive' (5-6 min)
      drivers = [],
      historyInterval = '1d',
      withBooks = true,
      withTrades = false,
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Mode: 'fast' (2-3 min, optimized) or 'comprehensive' (5-6 min, thorough)
          const forecastCard = await runAnalysis({
